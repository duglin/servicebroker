...
- For any OSBAPI request that is the result of an action taken by a Platform's
  user, there MUST be an associated `X-Broker-API-Originating-Identity` header
...
- Any OSBAPI request that is not associated with an action from a Platform's
  ...
  that HTTP request.
- If present on a request, the `X-Broker-API-Originating-Identity` header MUST
  contain the identify information for the Platform's user that took the
  action to cause the request to be sent.
...
change.
For example, if two different users modify an Instance, but the Platform
sends just one PATCH request to the Broker, the Broker would associate both
sets of changes with just one user. Whether it is correct or not for that
one user to be associated with both sets of changes (for auditing or
authorization purposes) is out of scope of this specification, but Platforms
need to be aware of the possible implications of doing such groupings.
